...
export const sanityClient = sanityConfigured()
  ? createClient({
    ...
    dataset,
    apiVersion,
    useCdn: !isDev && !readToken,
    perspective: isDev && readToken ? "previewDrafts" : "published",
...
export async function sanityFetch(query, params = {}) {
  if (!sanityClient) {
    if (isDev) {
      console.warn(
        "[sanity] Missing NEXT_PUBLIC_SANITY_PROJECT_ID. Studio posts will not appear. Copy env.example to .env.local, paste the Project ID from https://www.sanity.io/manage, and restart npm run dev.",
      );
    }
    return null;
  }
