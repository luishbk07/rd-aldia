...
import { getCronSnapshot, saveCronSnapshot } from "../cron-snapshot";
...
export function mlbScoresFallbackOptions(date = santoDomingoDate()) {
  ...
  return {
    ...
    async store() {
      try {
        return await getCronSnapshot("mlb-scores");
      } catch {
        return null;
      }
    },
    async primary() {
      ...
      saveCronSnapshot("mlb-scores", payload).catch(() => {});
      return payload;
    },
  };
}
...
export function mlbStandingsFallbackOptions(season = seasonYear()) {
  return {
    ...
    async store() {
      try {
        return await getCronSnapshot("mlb-standings");
      } catch {
        return null;
      }
    },
    async primary() {
      ...
      const payload = { season, divisions };
      saveCronSnapshot("mlb-standings", payload).catch(() => {});
      return payload;
    },
  };
}
