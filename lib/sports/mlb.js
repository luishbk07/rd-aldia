...
export async function getMlbScores(date = santoDomingoDate()) {
  return fetchWithFallback(mlbScoresFallbackOptions(date));
}
...
export async function getMlbStandings(season = seasonYear()) {
  return fetchWithFallback(mlbStandingsFallbackOptions(season));
}
...
export async function getMlbDominicanStars() {
  return fetchWithFallback(mlbDominicanStarsFallbackOptions());
}
...
export async function getMlbBundle() {
  const [standings, scores, stars] = await Promise.all([
    ...
    getMlbDominicanStars().catch((error) => ({
      players: [],
