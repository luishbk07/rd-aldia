...
export async function getCulturePosts() {
  ...
  const mapped = Array.isArray(rows)
    ...
    : [];
  const combined = uniqueBySlug([...mapped, ...CULTURE_ARTICLES]);
  return combined.length ? combined : CULTURE_ARTICLES;
}
...
export async function getFeaturedCulturePosts() {
  ...
  const mapped = Array.isArray(rows)
    ...
    : [];
  const combined = uniqueBySlug([...mapped, ...featuredCulture()]);
  return combined.length ? combined : featuredCulture();
}
