...
export async function fetchWeatherBundle() {
  ...
  try {
    await saveCronSnapshot("rd-weather", bundle);
  } catch {
    /* disk cache still holds the last good payload */
  }
  return bundle;
}

export async function storedWeatherBundle() {
  try {
    return await getCronSnapshot("rd-weather");
  } catch {
    return null;
  }
}
