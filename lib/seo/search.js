...
export function searchSite(rawQuery, extras = {}) {
  const query = String(rawQuery || "").trim().toLowerCase();
  if (query.length < 2) return [];

  const cultureItems = extras.culture || CULTURE_ARTICLES;
  const tourismItems = extras.tourism || DESTINATIONS;
  ...
  const culture = cultureItems
    .filter((article) =>
      match(
        `${article.title} ${article.excerpt} ${(article.body || []).join(" ")}`,
        query,
      ),
    )
    .map((article) => ({
